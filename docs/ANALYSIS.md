...
Известные function tables переведены в `.dw`, а остальные диапазоны вынесены по адресам в `assets/rom_data_START_END.bin`. Это сохраняет точное расположение и позволяет постепенно заменять каждый asset на структурированный исходник.
